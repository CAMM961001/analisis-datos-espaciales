...
setwd("~/Desktop/cursoITAM2023_MCD/clase4/data")
A <- read_csv("da_carpetas-de-investigacion-pgj-cdmx (1).csv")
...
tiempo <- c() #vector para guardar el tiempo de procesamiento auxiliar

a <- now() #Guarda este momento, en fecha y hora
a

#100 puntos
a <- now()
for (i in 1:100){
  distHaversine(c(B$longitud[i],B$latitud[i]),c(B$longitud[1],B$latitud[1]), r=6378137)
}
...
#Aumento del tiempo de procesamiento en cada ciclo

  #En cada ciclo, el ciclo calcula un problema 10 veces más grande:
plot(tiempo)
lines(tiempo)
...
#Guardamos los datos en el vecto total
total <- c()
...
for(i in 1:5){
  ...
  for(j in 1:nrow(B)){
    if((distHaversine(c(B$longitud[j],B$latitud[j]),c(B$longitud[i],B$latitud[i]), r=6378137) < 100)){
...
setwd("~/Downloads") #Movemos el archivo a la misma carpeta
rest <- read_csv("restaurantes_cdmx.csv") %>% data.frame()
...
#Capas de OSM
m %>% addTiles() %>% addProviderTiles(providers$OpenStreetMap.France)

#Color Negro
m %>% addTiles() %>% addProviderTiles(providers$CartoDB.DarkMatter)

#Capa más estandar de Carto
m %>% addTiles() %>% addProviderTiles(providers$CartoDB.VoyagerLabelsUnder)
...
icon.glyphicon <- makeAwesomeIcon(icon= 'subway', markerColor = 'lightgreen', iconColor = 'black',library = 'fa')
...
#Definimos el icono y sus caracteristicas
icon.amazon <- makeAwesomeIcon(icon= 'amazon', markerColor = 'pink', iconColor = 'black',library = 'fa')
...
shape_sub <- subset(shape, shape$nomgeo %in% c(
  "Tlalpan","Iztapalapa")) 
...
setwd("~/Downloads")
pasos <- read_csv("Master_20pasos_151.csv")
